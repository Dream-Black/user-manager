    ...
    <div class="header-right">
      ...
      <div ref="userDropdownRef" class="user-dropdown" @click="showUserMenu = !showUserMenu">
        ...
        <div v-if="showUserMenu" ref="userMenuRef" class="dropdown-menu">
          ...
          <button class="dropdown-item" type="button" @click="router.push('/settings')">设置</button>
          <button class="dropdown-item logout" type="button" @click="handleLogout">退出登录</button>
        </div>
      </div>
    </div>
...
import { useLayoutState, clearAuthState } from '@/composables/useLayoutState'
...
const notifications = ref([
  { id: 1, message: '新项目 "AI助手" 已被创建', time: '5分钟前', read: false },
  { id: 2, message: '任务 "设计登录页面" 已完成', time: '1小时前', read: false },
  { id: 3, message: '系统将于今晚23:00进行维护', time: '2小时前', read: true },
])
...
.dropdown-item { width: 100%; padding: var(--space-3) var(--space-4); font-size: var(--font-size-sm); color: var(--text-secondary); cursor: pointer; transition: all var(--transition-fast); background: transparent; border: none; text-align: left; }
...
.mark-read-btn { font-size: var(--font-size-xs); color: var(--primary-color); background: none; border: none; cursor: pointer; }
...
@media (max-width: 1024px) { .logo-text, .user-info, .search-shortcut { display: none; } .search-wrapper { max-width: 280px; } .notifications-panel { right: 0; width: min(360px, calc(100vw - 24px)); } }
@media (max-width: 768px) { .header { padding: 0 var(--space-3); gap: var(--space-3); } .search-wrapper { display: none; } .notifications-panel { width: calc(100vw - 24px); } }
</style>
